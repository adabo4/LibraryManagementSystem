...
async function returnOfTitle(id) {
    const data = await fetchData(`${API_URL}/api/RentalEntries/ReturnTitle/${id}`);
    function getId() {
    ...
    const dataset = getId();
    try {
        const res = await fetch(`${API_URL}/api/RentalEntries/ProlongTitle/${id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
...
fetchData(`${API_URL}/api/RentalEntries`).then((books) => createTable(books));
